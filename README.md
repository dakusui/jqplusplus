...
When `jq++` resolves `$extends` or `$includes`, it performs an index-wise recursive merge:
...
This matters when you want to override part of an array element without repeating the whole array.
    ...
    "http": [
      {
        ...
        "fault": {},
        "route": [{"destination": "v1"}]
      },
      {
        "route": [{"destination": "v2"}]
      }
    ]
  }
}
...
the first `http` element is merged with the parent's first element, and the second parent element is kept.
