...
A marked array whose parent does not define that key remains an unresolved delta, so mixin fragments can add to the arrays of their includers. The top-level result is checked after node-level inheritance; a token that remains unresolved is a configuration error. A marked array also errors if its inherited value is not an array, if `$super` and `$super*` are mixed, or if two marked arrays meet.
