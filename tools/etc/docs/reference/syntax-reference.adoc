...
|Array composition
|A marker in a child array explicitly combines it with the inherited array. `"$super"` splices inherited elements at its position; `"$super*"` pairs following elements index-wise.
|An unmarked array still replaces the inherited array. Markers are valid only as direct array elements; use `raw:` for a literal.
...
A link:../concepts/terminology.html#array-composition[marker] as a direct element of the child array opts into array composition instead.

*Splicing with `"$super"`*

`"$super"` substitutes the inherited elements at its position. Its position therefore encodes append, prepend, or wrap, and it may occur more than once.
...
`"$super*"` divides the child array into a literal prefix and a queue. Queue element _i_ pairs with inherited element _i_. Object pairs merge deeply, atom pairs use the child value, and an array pair replaces with the child array unless that child array is itself marked. Cross-kind pairs are errors. Unpaired elements on both sides remain in the result; `{}` is the object-pair pad that retains an inherited element unchanged.

[source,json]
----
// inherited: [ {"name":"a", "tls":false}, {"name":"b"} ]
{
  "servers": [
    {"name":"new"},
    "$super*",
...
// result: [ {"name":"new"}, {"name":"a", "tls":true}, {"name":"b"} ]
----

*Deltas and errors*

When no inherited array exists, a marked array remains a pending delta. This allows a `$includes` fragment such as `{"tags":["$super","platform"]}` to compose when the including document later supplies `tags`.
Grounding rejects a marker that remains pending in the final top-level object.

`"$super*"` may occur at most once in an array, and it cannot be mixed with `"$super"`.
A splice delta can compose with another splice delta, but any composition involving a pairing delta is an error.
Exact marker strings outside a direct array element are out of context; `$super` followed by a non-identifier character is an unknown marker.
`"$supervisor"` is an ordinary string, while `"raw:$super"` is the literal marker string.
