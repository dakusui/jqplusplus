...
|`$super` and `$super*` in an array
|Array composition
|A marked child array combines with its inherited array instead of replacing it.
`$super` splices inherited elements, while `$super*` pairs elements after the marker by index.
|A marker must be a direct array element.
An unmarked child array still replaces the inherited array.
...
A child array normally overrides the inherited array.
Place exactly one marker as a direct element of the child array to compose instead.

`$super` splices all inherited elements at the marker's position.
It can therefore append, prepend, or wrap them.
...
[source,json]
----
{ "$extends": ["P.json"], "m": ["$super", "z"] }
----

The result is `{ "m": ["a", "b", "z"] }`.

`$super*` keeps the elements before it as a literal prefix and pairs each element after it with the inherited element at the same index.
Object pairs merge recursively, while array and atom pairs take the child element.
Unpaired elements on either side remain in their original order.
Each pair must have the same kind: object, array, or atom.
Strings, numbers, booleans, and null are atoms.
A pair of different kinds is an error.
...
[source,json]
----
{ "$extends": ["P.json"], "m": ["$super*", {"b": 20}] }
----

The result is `{ "m": [{"a": 1, "b": 20}] }`.

Markers use the reserved `$super` namespace only in direct array-element position.
An unrecognized spelling in that position, or more than one marker in the same array, is an error.
Use `raw:$super` or `raw:$super*` when either literal string is required as an array element.

`$super*` resembles jq's recursive object-merge operator, but differs in two ways.
It pairs arrays by index even though jq rejects the corresponding operation on two arrays, and it takes the child value for atom pairs even though jq applies arithmetic to two atoms.

An array containing `$super*` cannot compose with an array containing either marker.
As a result, an override followed by an append is not expressible in one array composition.
Do not restate inherited elements as padding to simulate it: if the inherited array later grows, the intended appended element silently overrides the new inherited element instead.
Use `$super` when the goal is to append without overriding, or restructure the configuration so that the values have stable object keys.
