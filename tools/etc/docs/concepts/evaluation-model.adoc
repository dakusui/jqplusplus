...
|Array
|An unmarked child array replaces the inherited array.
A marked child array requests link:terminology.html[array composition]: `$super` splices inherited elements at each marker, and `$super*` pairs following elements with inherited elements by index.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

The default remains an override: arrays and scalars are replaced wholesale unless the child array explicitly contains a marker.
This preserves the link:design-principles.html[same-kind merge principle]: `$super*` pairs only values of the same kind, reporting a configuration error for a cross-kind pair.

`$super` splices the inherited elements at its position, so `["$super", "a"]`, `["a", "$super"]`, and `["a", "$super", "b"]` append, prepend, and wrap respectively.
Multiple `$super` markers are allowed.

`$super*` may occur once.
Its prefix remains literal; each queue element after the marker pairs with the inherited element at the same index.
Paired objects merge recursively, paired arrays replace unless the queue array is itself marked, and paired atoms are replaced by the queue atom.
Unpaired inherited elements or queue elements remain after the paired portion.

A marker is meaningful only as a direct array element.
`raw:` is the escape for a literal marker-family string, including a key or inheritance filename.
At a merge step with no inherited array, a marked array remains a pending delta so that cached fragments can contribute it to a later `$extends` or `$includes` merge.
Grounding after node-level inheritance rejects any marker that remains unresolved.

The two markers cannot be mixed in an array, and a delta involving `$super*` cannot compose with another marked array.
This restriction makes the result independent of how a configuration happens to be split into fragments.
...
Grounding validates the top-level object after all inheritance is resolved and before key-side evaluation.
It reports unresolved markers, marker-family strings outside direct array elements, and unknown marker spellings.
It deliberately runs after inheritance so that cached fragments can retain their pending deltas.
...
Pending array-composition deltas are deliberately retained in the stored result until an including or inheriting object supplies an inherited array.
...
- Keys present in both: if both values are objects, they are merged recursively using the same rule. Otherwise the child value replaces the parent value, except when a marked child array explicitly requests array composition.

An unmarked child array is not merged; it replaces the parent array completely.
A marked child array uses the array-composition rules above.
This follows the link:design-principles.html[same-kind merge principle]: every cross-kind pairing is a configuration error, while ordinary key overrides remain valid regardless of kind.
