...
|Array
|Replaced entirely unless the child is a marked array. `"$super"` splices the inherited elements at each marker position; `"$super*"` pairs the following elements with inherited elements by index.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

"Recursive" therefore applies only to nested objects, except for the explicit `"$super*"` array-composition form.
In jq{plus}{plus} terminology these wholesale replacements are _overrides_; object merging is the ordinary merge operation, and `"$super*"` invokes it only for paired objects.
This is the link:design-principles.html[same-kind merge principle].

For example, with an inherited `tags: ["x", "y"]`, a child value of `["before", "$super", "after"]` produces `["before", "x", "y", "after"]`.
Multiple `"$super"` markers are allowed.
`"$super*"` may appear once and cannot be mixed with `"$super"`; its prefix is literal and its queue is paired with inherited elements by index.
Cross-kind pairs are configuration errors.
See link:terminology.html#_array_composition_markers[Array Composition Markers] for the defined terms and escaping rules.
