...
`MergeNodes(parent, child)` combines two objects, with the child's values taking precedence.
...
|Array
|Replaced entirely by default — the child array overrides the parent.
A child array carrying an link:../reference/syntax-reference.html[array composition] token (`"$super"` or `"$super*"`) instead splices or pairwise-merges the inherited elements; this is opt-in per array.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

"Recursive" therefore applies to nested objects and, when explicitly requested with a token, to arrays; scalars and unmarked arrays are always replaced wholesale.
In jq{plus}{plus} terminology these wholesale replacements are _overrides_: the merges the engine performs are between two objects and, opt-in, between two arrays.
This is the link:design-principles.html[same-kind merge principle].

[NOTE]
====
Array composition was introduced by https://github.com/dakusui/jqplusplus/issues/74[issue #74].
A marked array whose inherited key is absent carries as an unresolved _delta_; a token still unresolved after stage 4 is reported as a configuration error by the final validation sweep described below.
====
...
Once file-level and node-level inheritance are both settled for the top-level object — and before key-side evaluation begins — jq{plus}{plus} walks the entire result once and validates array composition:

- Any remaining `"$super"` / `"$super*"` token is _dangling merge intent_: nothing is left for it to compose with, so it is reported as a configuration error rather than silently splicing nothing.
- Any other array string element extending `$super` with a non-identifier character (such as `"$super?"`) is an unknown token in the reserved namespace and is likewise an error.

The sweep runs on the top-level object only.
Per-file cached resolutions in the node pool keep their tokens, so deltas survive into later merges — this is what allows an `$includes` fragment to carry `["$super", ...]` and append to whatever includes it.
The sweep visits object keys in sorted order, so the first reported error is deterministic.
...
`MergeNodes(parent, child)` performs a deep merge:
...
- Keys present in both: if both values are objects, they are merged recursively using the same rule; if both values are arrays and the child array carries an array composition token, they are composed; otherwise the child value replaces the parent value entirely.

By default arrays are therefore **not** merged—a child array replaces the parent array completely unless the child array opts in with a token.
This follows the link:design-principles.html[same-kind merge principle]: merging is defined only between values of the same kind, and — absent explicit merge intent — every cross-kind combination is an override that replaces the value rather than a merge.
Where a token requests a merge and the inherited value is of a different kind, jq{plus}{plus} reports a configuration error.
