...
|Array
|Replaced entirely by default. A child array can opt in to composition with a direct `$super` or `$super*` element.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

"Recursive" therefore applies automatically only to nested objects; unmarked arrays and scalars are replaced wholesale.
In jq{plus}{plus} terminology these wholesale replacements are _overrides_.
This is the link:design-principles.html[same-kind merge principle].

*Explicit array composition*

An unmarked child array still replaces the inherited array. To opt in, use one of these exact strings as a direct child-array element:

* `$super` splices every inherited element at its position. `["$super", "local"]` appends and `["local", "$super"]` prepends; multiple occurrences are allowed.
* `$super*` occurs once and starts positional, pairwise merging. Elements before it are a literal prefix; elements after it are paired with inherited elements by index. Object pairs are deeply merged, array pairs recurse only when the child array is itself marked, and atom pairs use the child value. Unpaired inherited elements remain before unpaired child elements.

Composition tokens are resolved for both `$extends` and `$includes`, at file and node level. A token that has no inherited array may pass through an intermediate fragment as a delta, but it is an error if still unresolved after node-level inheritance. `$super` splice deltas compose associatively; any encounter involving an unresolved `$super*` pairwise delta is an error. Cross-kind pairs, a non-array inherited value, and mixed `$super` / `$super*` markers are also errors. See link:../reference/syntax-reference.html#array-composition-super-and-super[Array composition] for complete syntax and escaping rules.
...
This means unmarked arrays are **not** merged—a child array replaces the parent array completely.
Only nested objects receive recursive treatment automatically. A child array can explicitly request splice or positional pairwise composition with `$super` or `$super*`; see link:../reference/syntax-reference.html#array-composition-super-and-super[Array composition].
This follows the link:design-principles.html[same-kind merge principle]: explicit pairwise merges are defined only between values of the same kind, while ordinary inheritance remains an override.
