...
|Array
|Replaced entirely when unmarked. A child array may opt into composition with direct `"$super"` or `"$super*"` elements; see <<explicit-array-composition>>.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

"Recursive" therefore applies to nested objects by default. Array composition is an explicit operation at the child array site; scalar values remain wholesale overrides unless they are paired by `$super*`.
This is the link:design-principles.html[same-kind merge principle].

[NOTE]
====
Array composition is always opt-in. An unmarked child array still replaces its inherited array, preserving the original behavior.
The exact marker strings are reserved: `"$super"` splices inherited elements at its position, while `"$super*"` starts an index-wise pairwise merge.
If a marked array has no inherited array to ground it, or if a reserved marker is malformed, resolution reports a configuration error.
====
...
==== Final array-merge grounding

After node-level inheritance, but before key-side evaluation, the top-level result is checked for unresolved array-merge intent.
This late check lets cached parent objects carry a `"$super"` delta through several inheritance or inclusion steps before a concrete inherited array is supplied.

The marker namespace is validated during the same walk:

* `"$super"` and `"$super*"` are defined only as direct elements of an array during inheritance merging.
* A marker inside a nested array does not splice the surrounding array; if it remains unresolved, it is reported as dangling.
* Strings such as `"$supervisor"` are ordinary literals because the suffix begins with an identifier character.
* A `$super`-family spelling followed by punctuation, such as `"$super?"` or `"$super*(name)"`, is an unknown-token error. Use `raw:$super` when the literal value is intended.

Values produced later by `eval:` are not rescanned by this validation pass.
...
By default, arrays are **not** merged—a child array replaces the parent array completely.
Only nested objects receive the recursive treatment unless a child array explicitly opts into composition with `$super` or `$super*`.
...
Explicit array markers do not change the default `mergeObjects` behavior. They request a merge only at the marked child array, and therefore activate the same-kind checks for that operation.

[[explicit-array-composition]]
==== Explicit array composition

`"$super"` is a positional splice marker. It is replaced by the inherited array's elements at its position, so `["$super", "child"]` appends, `["child", "$super"]` prepends, and `["before", "$super", "after"]` wraps.
Multiple splice markers are allowed. Splice deltas compose associatively, which lets an included fragment carry `"$super"` until a later merge supplies the inherited array.

`"$super*"` is a positional pairwise marker. Elements before it are a literal prefix; elements after it form a queue paired with inherited elements by index. Paired values follow these rules:

* object with object: deep object merge, with the child fields taking precedence;
* array with array: the child replaces it unless that child array is itself marked, in which case the operation recurses;
* atom with atom: the child wins;
* any cross-kind pair: configuration error.

Unpaired inherited elements remain after the paired region, and unpaired child queue elements form a literal tail. An empty object is consequently a useful keep-inherited pad for an inherited object element.
Only one `$super*` marker is allowed in an array, and `$super*` cannot be combined with another marked array during v1 resolution because pairwise deltas do not compose associatively.
