...
|Array
|Replaced entirely by default. A child may opt in to array composition with a marker; see <<array-composition>>.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

"Recursive" therefore applies only to nested objects unless a child explicitly opts in to array composition.
...
Array replacement remains the default. To opt in, write a marker as a direct
element of the child array. `$super` splices all inherited elements at its
position, so `["before", "$super", "after"]` wraps the inherited array.
More than one `$super` is permitted.

`$super*` may occur exactly once. Elements before it are a literal prefix;
elements after it pair with inherited elements by index. Paired objects
deep-merge, paired atoms are replaced by the child, and paired arrays are
replaced unless the child array is marked recursively. Cross-kind pairs are
errors. For example, `["new", "$super*", {"tls": true}, {}]` prepends `new`,
adds `tls` to the first inherited object, and preserves the second object.

If no inherited value exists yet, a marked array remains a delta for a later
inheritance step; this makes reusable `$includes` fragments composable. A
`$super*` delta cannot compose with another marked array, because its positional
queue would make the result grouping-dependent. At final grounding, unresolved
markers, markers outside direct array-element positions, and unknown `$super`
family spellings are errors. Use `raw:$super` for a literal value or key.
...
After node-level inheritance, jq{plus}{plus} grounds the final top-level object.
This validates array-composition markers before key-side evaluation. Cached
fragments are not grounded, so their unresolved deltas can compose when they
...
This means arrays are **not** merged by default—a child array replaces the parent array completely unless it contains an array-composition marker.
Only nested objects receive the recursive treatment without an explicit marker.
Array pairing follows the link:design-principles.html[same-kind merge principle]: cross-kind pairs are configuration errors rather than overrides.
