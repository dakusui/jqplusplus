...
A marked child array uses `$super` to splice inherited elements at the marker or `$super*` to pair elements after the marker by index.

|Scalar (string, number, boolean, null)
|Replaced entirely — the child value overrides the parent value.
|===

"Recursive" therefore applies only to nested objects and to object pairs created by `$super*` array composition.
In jq{plus}{plus} terminology, a child value replacing an inherited value is an _override_.
This is the link:design-principles.html[same-kind merge principle].

See the link:../reference/syntax-reference.html#_array_composition[Syntax Reference] for the marker syntax and its constraints.
