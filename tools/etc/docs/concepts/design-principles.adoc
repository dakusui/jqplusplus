...
Ordinary inheritance still treats arrays and atoms as overrides. The explicit `"$super*"` array operation is the exception: it requests a same-kind pairwise merge at its marked site, recursively applying object, array, and atom rules while rejecting cross-kind pairs.
The sibling `"$super"` operation is a splice rather than a pairwise merge, but it also requires an inherited array and preserves array kind throughout composition.
