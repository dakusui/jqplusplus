...
## 4. Documentation, compatibility policy, and release record
...
- [ ] 4.4 Publish the pre-1.0 compatibility policy in `tools/etc/docs/concepts/design-principles.adoc` and a concise, prominent stability notice in `README.md`: while the project remains at `0.0.x` with limited adoption, an incompatible change requires documentation but no deprecation period, dual syntax, automated migration, or rollback procedure unless explicitly required; reconcile the terminology and syntax-reference statements that currently reserve new keyword families for a major-version change, and verify all policy statements agree
- [ ] 4.5 Replace documented `_`-prefixed holder patterns with `$aux`, correct the `yq++` description in `README.md` and user documentation, retain legitimate underscore-prefixed data examples, and record both concrete compatibility breaks in `CHANGELOG.md`; verify the documentation identifies `raw:$aux` as the escape for literal output and `$aux` as the processing-only mechanism without presenting a staged migration or rollback plan
