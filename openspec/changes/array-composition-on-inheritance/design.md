...
→ Mitigation: `raw:$super` and a changelog entry backfilled under `v0.0.50`, the release that introduced marker recognition. Recorded in the proposal rather than described as a change with no compatibility effect.
...
An unmarked array composes exactly as it does today, so no configuration changes meaning through composition. The one exception is the literal `$super` array element described above, whose marker recognition shipped in `v0.0.50` and is recorded there with a changelog entry pointing at `raw:` rather than as a change with no compatibility effect.
